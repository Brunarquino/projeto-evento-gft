...
import br.com.gft.services.RoleService;
...
public class UsuarioController {
	...
	/*
	 * @RequestMapping(method = RequestMethod.POST, path = "entrar") public
	 * ModelAndView entrarComoAdiministrador(Usuario usuario, RedirectAttributes ra)
	 * {
	 * 
	 * ModelAndView mv; try { Usuario usuarioEncontrado =
	 * usuarioService.obterUsuarioPelasQuatroLetras(usuario.getQuatroLetras()); if
	 * (!usuarioEncontrado.getSenha().equals(usuario.getSenha())) { mv = new
	 * ModelAndView("area-acesso-adm/usuario/form/login.html");
	 * usuario.setSenha(null); mv.addObject("usuario", usuario);
	 * mv.addObject("mensagem", "Senha incorreta!"); mv.addObject("cor", "danger");
	 * 
	 * } else { mv = new ModelAndView("redirect:/usuario/home");
	 * ra.addFlashAttribute("usuario", usuarioEncontrado);
	 * ra.addFlashAttribute("mensagem", "Entrou como administrador!"); }
	 * 
	 * } catch (Exception e) { mv = new
	 * ModelAndView("area-acesso-adm/usuario/form/login.html");
	 * mv.addObject("usuario", new Usuario()); mv.addObject("mensagem",
	 * e.getMessage()); mv.addObject("cor", "danger"); }
	 * 
	 * return mv;
	 * 
	 * }
	...
	public ModelAndView salvarUsuarios(@Valid Usuario usuario, BindingResult bindingResult, RedirectAttributes ra) {
		...
		boolean usuarioComEsseEmailJaExiste = usuarioService.verificarSeUsuarioComEsseEmailJaExiste(usuario.getEmail())
				.isEmpty();
		...
		if (!bindingResult.hasErrors() && usuarioComEssasQuatroLetrasJaExiste && usuarioComEsseEmailJaExiste) {
			...
			RoleModel role = roleService.criarRoleAdminSeNaoExistir();
